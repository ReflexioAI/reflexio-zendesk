...
class ProfileGenerationService(
    ...
    def _finalize_extracted_items(self, all_new_profiles: list[UserProfile]) -> None:
        ...
        # the dedup path). Capture the ids that ACTUALLY committed so the legacy
        # change-log below records only real removals (commit-atomic invariant).
        committed_ids: list[str] = []
        if existing_ids_to_delete:
            if not request_id:
                # An empty request_id makes the removal unreconstructable (the legacy
                # log and lineage events are keyed on it). Fail loud and skip removal
                # entirely — never silently hard-delete.
                capture_anomaly(
                ...
                try:
                    committed_ids = self.storage.supersede_profiles_by_ids(  # type: ignore[reportOptionalMemberAccess]
                        user_id=user_id,
                    ...
                    # Ensure NO phantom removal is logged below.
                    committed_ids = []

        # Create profile changelog post-deduplication. The "removed" entries are built
        # from what supersede actually committed, NOT the deduplicator's intent, so a
        # failed/skipped/partial supersede never produces a phantom legacy removal.
        committed_set = set(committed_ids)
        removed_for_log = [
            p for p in superseded_profiles if p.profile_id in committed_set
        ]
        if all_new_profiles or removed_for_log:
            try:
                profile_change_log = ProfileChangeLog(
                    ...
                    removed_profiles=removed_for_log,
                    mentioned_profiles=[],
                )
