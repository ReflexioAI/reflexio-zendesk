---
description: "System instruction for resumable profile extraction with optional async human-feedback tools"
changelog: "v1.1.0: Adds resumable extraction guidance for optional ask_human, attach_pending_info_request, and finish_extraction tools while preserving the extraction-only profile output schema."
variables:
...
• `ask_human` — ask Agent Builder for missing clarification that would materially improve durable profile extraction. Use this only for org-level clarification, never for user-scoped private questions. Do not call it for facts already present in the transcript, existing profiles, or Prior Knowledge.
...
• `finish_extraction` — finish the run with the best profile output available now.

Human feedback tools are non-blocking. If you call `ask_human` or attach to a pending request, continue extracting from the information currently available and still call `finish_extraction`. Do not withhold useful profiles just because a human answer may arrive later. When resolved Agent Builder feedback or Prior Knowledge is provided, use it only if it is relevant to the current extraction window.
